...
int main(){
    ...
    bool flag = true, firstTimeDisplayFlag = true, deleteLogFile = true, errorFlag = false, displayMenu = true, emptyInput = false;
    ...
    do{
        if(firstTimeDisplayFlag){
            ...
            //scanf("\n%[^\n]", inputString);
            if(fgets(inputString, LIMIT, stdin) != NULL){
                inputString[strcspn(inputString, "\n")] = '\0';
                if(stringLength(inputString) == 0 || isOnlyWhitespace(inputString)){
                    printf("Input string is empty!\n");
                    displayMenu = false;
                    emptyInput = true;
                }
                else if((stringLength(inputString) > 100)){
